...
describe 'Bulk Categorization API', type: :request do
  let!(:tx1) { create(:transaction, description: 'A', amount: 1, category: 'Old', date: Date.today) }
  let!(:tx2) { create(:transaction, description: 'B', amount: 2, category: 'Old', date: Date.today) }
  let!(:tx3) { create(:transaction, description: 'C', amount: 3, category: 'Other', date: Date.today) }
