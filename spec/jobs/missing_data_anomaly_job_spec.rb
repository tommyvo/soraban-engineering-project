...
describe MissingDataAnomalyJob, type: :job do
  ...
  before do
    ...
    allow(anomalies).to receive(:where).with(anomaly_type: 'MissingData').and_return(anomalies)
    allow(anomalies).to receive(:destroy_all)
  end

  it 'calls the service and creates an anomaly if result is present' do
    result = { anomaly_type: 'MissingData', reason: 'missing' }
    expect(MissingDataAnomaly).to receive(:call).with(transaction).and_return(result)
    expect(anomalies).to receive(:create!).with(anomaly_type: 'MissingData', reason: 'missing')
    described_class.perform_now(42)
  end

  it 'calls the service and does not create an anomaly if result is nil' do
    expect(MissingDataAnomaly).to receive(:call).with(transaction).and_return(nil)
